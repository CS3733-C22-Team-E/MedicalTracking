...
public class MedicalEquipmentServiceRequestManager implements IManager<MedicalEquipmentServiceRequest>{
    ...
    public MedicalEquipmentServiceRequest get(String id) {
        ...
        try{
            ...
            while(rset.next()) {
                result = new MedicalEquipmentServiceRequest(rset.getString("id"), rset.getString("id"),rset.getString("id"),rset.getString("id"),rset.getString("id"),rset.getString("id"),rset.getString("id"),rset.getString("id"),rset.getString("id"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public LinkedList<MedicalEquipmentServiceRequest> getAll() {
        ...
        try{
            ...
            while(rset.next()) {
                result.add(new MedicalEquipmentServiceRequest(rset.getString("id"), rset.getString("id"),rset.getString("id"),rset.getString("id"),rset.getString("id"),rset.getString("id"),rset.getString("id"),rset.getString("id"),rset.getString("id")));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void insert(MedicalEquipmentServiceRequest newObject) {
        String insertQuery = "INSERT INTO EQUIPMENTSERVICEREQUEST VALUES('"
                + newObject.getId()
                + "','"
    ...
    public void update(MedicalEquipmentServiceRequest updatedObject) {
        String updateQuery = "UPDATE LOCATIONS SET patient = '" + updatedObject.getPatient()
                + "', room = '" + updatedObject.getRoom()
                + "', startTime = '" + updatedObject.getStartTIme()
                + "', endTime = '" + updatedObject.getEndTime()
                + "', date = '" + updatedObject.getDate()
                + "', assignee = '" + updatedObject.getAssignee()
                + "', equipment = '" + updatedObject.getEquipment()
                + "', status = '" + updatedObject.getStatus()
                + "' WHERE id = '"
                + updatedObject.getId()
                + "'";
